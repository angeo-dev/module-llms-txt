...
use Angeo\LlmsTxt\Model\Jsonl\Providers\StoreProvider;
use Angeo\LlmsTxt\Model\Jsonl\Providers\CmsPageProvider;
...
class JsonlGeneratorTest extends TestCase
{
    ...
    public function testGenerateWritesFileForActiveStore(): void
    {
        ...
        $this->directory
            ...
            ->with('llms_default.jsonl')
            ->willReturn('/media/llms_default.jsonl');

        $this->fileIo
            ->expects($this->once())
            ->method('write')
            ->with('/media/llms_default.jsonl', 'test-content', 0775);

        $generator = new JsonlGenerator(
            $this->storeManager,
            $this->filesystem,
            $this->fileIo,
            [$provider]
    ...
    public function testMultipleProvidersAreConcatenated(): void
    {
        ...
        $this->fileIo
            ->expects($this->once())
            ->method('write')
            ->with('/media/llms_default.jsonl', 'part1part2', 0775);
